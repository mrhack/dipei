exports.init = function(grunt) {
  ...
  exports.jsParser = function( src , tar , id , options) {
    options = options || {};
    ...
    if (!meta) {
      //grunt.log.warn('found non cmd module "' + src + '"');
      // do nothing
      return;
    }

    if (meta.id) {
      grunt.log.warn('found id in "' + src + '"');
      grunt.file.write(tar, data);
      return;
    }
    var deps = parseDependencies(src, options);
    ...
    astCache = ast.modify(astCache, {
      id: unixy(options.idleading + id.replace(/\.js$/, '')),
      dependencies: deps,
      require: function(v) {
    ...
    grunt.log.writeln('Transport ' + src + ' -> ' + tar);
    grunt.file.write(tar, data);

    if (!options.debug) {
      return;
    }
    var dest = tar.replace(/\.js$/, '-debug.js');
